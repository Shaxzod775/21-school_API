...
#     """Shows the user's subscription status."""
#     chat_id = update.effective_chat.id
#     user_id = update.effective_user.id

#     await context.bot.send_message(chat_id=chat_id, text="Введите ваш логин:")
#     login_message = await context.bot.get_updates(timeout=60)
#     login = login_message[-1].message.text if login_message else None

#     await context.bot.send_message(chat_id=chat_id, text="Введите название проекта:")
#     project_message = await context.bot.get_updates(timeout=60)
#     project_name = project_message[-1].message.text if project_message else None

#     if not login or not project_name:
#         await context.bot.send_message(chat_id=chat_id, text="Не удалось получить логин или название проекта.")
#         return

#     project_id = INTENSIVE.get(project_name.capitalize())
#     if not project_id:
#         await context.bot.send_message(chat_id=chat_id, text="Проект не найден.")
#         return
...
#             await context.bot.send_message(chat_id=chat_id, text=f"Проект: {title}\nСтатус: {status}\nДата завершения: {completionDateTime}\nКоманда: {teamMembers}\nПроцент завершения: {finalPercentage}")
#         else:
#             raise Exception(f"Ошибка при получении подписки: {response.status_code}")

#     except Exception as e:
#         await context.bot.send_message(chat_id=chat_id, text=f"Не удалось получить подписку пользователя: {e}")
#         return
