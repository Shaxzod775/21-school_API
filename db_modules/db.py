...
def get_all_users_chatId(db_path):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()

    cursor.execute("SELECT chatId FROM users")
    chatIds = [row[0] for row in cursor.fetchall()]
...
def init_table_posts():
    try:
        conn = sqlite3.connect('../bot_databases/posts.db')
        cursor = conn.cursor()
...
def get_number_of_users_in_bot(db_path):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
...
def get_number_of_users_authenticated_in_bot(db_path):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()

    cursor.execute("SELECT COUNT(*) FROM users WHERE edu_username IS NOT NULL AND edu_password IS NOT NULL")
    count = cursor.fetchone()[0]
